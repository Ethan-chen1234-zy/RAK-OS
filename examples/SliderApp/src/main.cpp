/** RAKOS SliderApp — slider widgets demo (ota_1 @ 0x400000) */
#include <Arduino.h>
#include <lvgl.h>
...
static void slider_event_cb(lv_event_t *e) {
    ...
    if (lbl) {
        lv_label_set_text_fmt(lbl, "%ld", (long)val);
    }
}
...
static void build_ui() {
    ...
    lv_label_set_text(title, "Slider Demo");
    ...
    lv_label_set_text(bright_label, "50");
    ...
    lv_slider_set_value(s1, 50, LV_ANIM_OFF);
    lv_obj_align(s1, LV_ALIGN_TOP_MID, 0, 132);
    lv_obj_add_event_cb(s1, slider_event_cb, LV_EVENT_VALUE_CHANGED, bright_label);
    ...
    lv_label_set_text(vol_label, "30");
    ...
    lv_slider_set_value(s2, 30, LV_ANIM_OFF);
    lv_obj_align(s2, LV_ALIGN_TOP_MID, 0, 222);
    lv_obj_add_event_cb(s2, slider_event_cb, LV_EVENT_VALUE_CHANGED, vol_label);
